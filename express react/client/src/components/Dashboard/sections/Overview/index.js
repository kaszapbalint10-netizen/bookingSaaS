...
const Overview = ({
  ...
  const heroMultiSeries = useMemo(() => {
    ...
    // Yearly: töltsük fel minden napot startDate-től ma-ig 0-val, ha nincs adat
    if (timeRange === 'year') {
      const filled = [];
    ...
    // Week / month: csak a meglévő napok a tartományban, fallback egy üres napra, ha nincs adat
...
<section className="vision-grid">
        <article className="vision-card hero" onClick={() => openPanel('openingHours')}>
          <div className="card-body calendar">
            <PricingCalendar data={safeCalendar} isExpanded={false} isDarkMode={false} isCompact />
          </div>
        </article>

        {/* Line chart card -> Árképezési naptár panel */}
        <article className="vision-card calendar" onClick={() => openPanel('calendar')}>
          <div className="card-body calendar-chart">
